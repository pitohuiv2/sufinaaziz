...
@Component({
  selector: 'app-daftar-pasien-farmasi',
  templateUrl: './daftar-pasien-farmasi.component.html',
  styleUrls: ['./daftar-pasien-farmasi.component.scss'],
  providers: [ConfirmationService]
})
export class DaftarPasienFarmasiComponent implements OnInit {
  page: number;
  rows: number;
  selected: any;
  dataTable: any[];
  ...
  LoadCache() {
    var chacePeriode = this.cacheHelper.get('DaftarPasienFarmasiCache');
  ...
  getData() {
    ...
    this.cacheHelper.set('DaftarPasienFarmasiCache', chacePeriode);
  ...
  transaksiLayanan() {
    ...
    var jenisdata = "pasien";
    var arrStr = {
      0: "",
      1: "",
    }
    this.cacheHelper.set('rincianPelayananFarmasiCache', arrStr);
    this.router.navigate(['transaksi-pelayanan-apotik', this.selected.norec, jenisdata])
  }
  ...
  inputObat() {
    ...
    this.router.navigate(['input-resep-apotik', this.selected.norec, this.item.dataRuanganApd.norec_apd]);
  }
  filter() {
    this.popFilter = true
  }
