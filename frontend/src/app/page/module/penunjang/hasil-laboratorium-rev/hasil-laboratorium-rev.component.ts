...
@Component({
  selector: 'app-hasil-laboratorium-rev',
  templateUrl: './hasil-laboratorium-rev.component.html',
  styleUrls: ['./hasil-laboratorium-rev.component.scss'],
  providers: [ConfirmationService, MessageService]
})
export class HasilLaboratoriumRevComponent implements OnInit {
  params: any = {};
  currentNorecPD: any;
  currentNorecAPD: any;
  ...
  cari() {
    this.apiService.get("penunjang/get-hasil-lab-manual-rev?norec_apd=" + this.currentNorecAPD +
      '&objectjeniskelaminfk=' + this.item.pasien.objectjeniskelaminfk + '&umur=' + this.item.umurDay).subscribe(res => {
        this.listSatuan = []
        this.listSatuan = res.satuan
  ...
  onRowSelect(e) {
    ...
    if (e.satuanstandar != null) {
      for (let x = 0; x < this.listSatuan.length; x++) {
        const element = this.listSatuan[x];
        if (element.satuanstandar == e.satuanstandar) {
          this.item.satuanstandar = element
        }
      }
    }
  ...
  saveNilai() {
    ...
    this.item.mpidL = null
    this.item.mpidP = null
    this.apiService.get("penunjang/get-for-update-nilainormal-rev?produkfk=" + this.item.objectprodukfk).subscribe(data => {
        if (data.length > 0) {
          for (var i = data.length - 1; i >= 0; i--) {
            if (data[i].jkid == 1) {
              this.item.mpidL =  data[i].id
  ...
  simpanUpdateNilaiNormal() {
    if (this.item.nilaiTextP == undefined) {
      this.alertService.warn('Info', 'Nilai Text harus di isi')
      return
    }
    var objSave = {
      "l": {
        id: this.item.mpidL != null ? this.item.mpidL : '',
        nilaimax: this.item.nilaimaxL ? this.item.nilaimaxL : null,
        nilaimin: this.item.nilaiminL ? this.item.nilaiminL : null,
        nilaitext: this.item.nilaiTextL,
        produkidfk: this.item.objectprodukfk,
        ssid: this.item.satuanstandar != undefined ? this.item.satuanstandar.id : null,
      },
      "p": {
        id: this.item.mpidP != null ? this.item.mpidP : '',
        nilaimax: this.item.nilaimaxP ? this.item.nilaimaxP : null,
        nilaimin: this.item.nilaiminP ? this.item.nilaiminP : null,
        nilaitext: this.item.nilaiTextP,
        produkidfk: this.item.objectprodukfk,
        ssid: this.item.satuanstandar != undefined ? this.item.satuanstandar.id : null,
      }
    }
    this.apiService.post('penunjang/save-update-nilainormal-rev', objSave).subscribe(e => {
      this.popupEntry = false
      this.isSelected = false;
      if (this.item.satuanstandar != undefined) {
        this.selected.satuanstandar = this.item.satuanstandar.satuanstandar
      }
