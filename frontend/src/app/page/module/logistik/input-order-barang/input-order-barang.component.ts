...
@Component({
  selector: 'app-input-order-barang',
  templateUrl: './input-order-barang.component.html',
  styleUrls: ['./input-order-barang.component.scss'],
  providers: [ConfirmationService]
})
export class InputOrderBarangComponent implements OnInit {
  norec_data: any;
  params: any = {};
  item: any = {};
  listRuangan: any[];
  ...
  ngOnInit(): void { 
    ...
    this.item.tglOrder = this.dateNow;
    this.loadColumn();
    this.loadCombo();
    this.firstLoad();
  }
  ...
  loadColumn() {
    this.columnGrid = [
      ...
      { field: 'jumlah', header: 'Qty Order', width: "140px" }
    ];
  }
  ...
  loadData() {
    if (this.norec_data != "-") {
      this.apiService.get("logistik/get-detail-order-barang-ruangan?norecOrder=" + this.norec_data).subscribe(dataLoad => {        
        var datahead = dataLoad.head;
        var detail = dataLoad.detail;
        this.item.nomorOrder = datahead.noorder;
        this.item.tglOrder = moment(datahead.tglorder).format("YYYY-MM-DD HH:mm");
        this.item.dataJenisKirim = { id: datahead.jeniskirimfk, jeniskirim: datahead.jeniskirim };
        this.item.ruangan = { id: datahead.objectruanganasalfk, namaruangan: datahead.namaruanganasal };
        this.listRuanganAll = [{
          id: datahead.objectruangantujuanfk, namaruangan: datahead.namaruangantujuan
        }];
        this.item.dataRuanganAll = { id: datahead.objectruangantujuanfk, namaruangan: datahead.namaruangantujuan }
        this.item.keteranganOrder = datahead.keteranganorder;
        this.data2 = detail;
        var total: any = 0;
  ...
  GETKONVERSI() {
    ...
    this.apiService.get("general/get-produkdetail-general?" +
      ...
      "&ruanganfk=" + this.item.dataRuanganAll.id).subscribe(dat => {
    ...
    if (this.item.no != undefined) {
      for (var i = this.data2.length - 1; i >= 0; i--) {
        if (this.data2[i].no == this.item.no) {
          ...
          data.harganetto = String(this.item.hargaSatuan)
      ...
      data = {
        ...
        harganetto: String(this.item.hargaSatuan),
  ...
  save() {
    if (this.item.ruangan == undefined) {
      this.alertService.error("Info", "Pilih Ruangan Pengorder!")
      return
    }
    if (this.item.dataRuanganAll == undefined) {
      this.alertService.error("Info", "Pilih Ruangan Tujuan!")
      return
    }
    ...
    if (this.item.keteranganOrder == undefined) {
    ...
    if (this.item.keteranganOrder != undefined || this.item.keteranganOrder != '') {
      keterangan = this.item.keteranganOrder
    }
    var strukorder = {
      norecorder: this.norec_data,
      pegawaiorderfk: this.dataLogin.pegawai.id,
      ruanganfk: this.item.ruangan.id,
      ruangantujuanfk: this.item.dataRuanganAll.id,
      jenispermintaanfk: this.item.dataJenisKirim.id,
      keteranganorder: keterangan,
      qtyjenisproduk: this.data2.length,
      tglorder: moment(this.item.tglOrder).format('YYYY-MM-DD hh:mm'),
    }
    var objSave =
    {
    ...
    this.apiService.post('logistik/save-order-barang-ruangan', objSave).subscribe(dataSave => {
      this.apiService.postLog('Simpan Order Barang Ruangan', 'Norec transaksiorder', dataSave.data.norec,
        'Simpan Order Barang Ruangan Dengan Noorder - ' + dataSave.data.noorder).subscribe(res => { })
      this.Kosongkan();
      this.data2 = []
      this.dataSource = undefined
      window.history.back();
