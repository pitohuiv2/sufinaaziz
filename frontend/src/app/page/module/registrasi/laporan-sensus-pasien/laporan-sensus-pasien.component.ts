...
@Component({
  selector: 'app-laporan-sensus-pasien',
  templateUrl: './laporan-sensus-pasien.component.html',
  styleUrls: ['./laporan-sensus-pasien.component.scss'],
  providers: [ConfirmationService, MessageService]
})
export class LaporanSensusPasienComponent implements OnInit {
  page: number;
  rows: number;
  column: any[];
  selected: any;
  ...
  ngOnInit(): void {
    this.column = [
      ...
      { field: 'baru', header: 'Baru', width: "85px" },
      { field: 'lama', header: 'Lama', width: "85px" },
      { field: 'tekanandarah', header: 'Tekanan Darah', width: "150px" },
      { field: 'bb_tb', header: 'BB/TB', width: "140px" },
      { field: 'keluhan', header: 'Keluhan', width: "140px" },
      { field: 'kddiagnosa', header: 'Diagnosa', width: "180px" },
      { field: 'terapi', header: 'Therapy', width: "180px" },
      { field: 'statuspulang', header: 'Status', width: "180px" },
    ];
  ...
  getData() {
    ...
    this.apiService.get("laporan/get-data-lap-sensus-pasien?"
  ...
  exportExcel() {    
    ...
    window.open(Config.get().apiBackend + "print/cetak-laporan-sensus-pasien?" +
      "&tglAwal=" + tglAwal + "&tglAkhir=" + tglAkhir +
      idDept + idRuangan + idTipePasien + paramRuang + "&kodeprofile=" + profile + "&namauser=" + namauser);
  }
}