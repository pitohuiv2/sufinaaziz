...
export class PasienBaruComponent implements OnInit {
  ...
  init() {
    ...
    if (this.nocmIgd != undefined) {
      ...
      if (chacePeriode != undefined) {
        ...
        this.item.tempatLahir = chacePeriode.tempatlahir.toUpperCase();
        this.item.tglLahir = moment(chacePeriode.tgllahir).format('DD-MM-YYYY');//moment(this.item.tglLahir).format('YYYY-MM-DD HH:mm');//;
  ...
  getNik(jenis, query) {
    let json = {
      "url": "Peserta/" + jenis + "/" + query + "/tglSEP/" + moment(new Date()).format('YYYY-MM-DD'),
      "method": "GET",
      "data": null
    }
    this.apiService.postNonMessage("bridging/bpjs/tools", json).subscribe(e => {
      if (e.metaData.code == "200") {
        ...
        this.item.namaPasien = e.response.peserta.nama
        this.item.noIdentitas = e.response.peserta.nik
        this.item.noBpjs = e.response.peserta.noKartu
        this.item.noHp = e.response.peserta.noTelepon
        this.item.tglLahir = new Date(e.response.peserta.tglLahir)

        if (e.response.peserta.sex == 'P') {
          this.item.jenisKelamin = this.listDataJenisKelamin[1]
        } else {
          this.item.jenisKelamin = this.listDataJenisKelamin[0]
        }
  ...
  save() {    
    ...
    // var nomor = this.item.noIdentitas;
    // if (nomor.length > 16) {
    //   this.alertService.error("Info", "Nomor Identitas / NIK Tidak Boleh Lebih Dari 16 Karakter!");
    //   return; 
    // }   
    ...
    this.apiService.post('registrasi/save-pasien', postJson)
      .subscribe(
        res => {
          if (this.item.noRecReservasi != undefined) {
            ...
            this.apiService.post('reservasionline/update-nocmfk-antrian-registrasi', jsonss).subscribe(e => {
            })
          }
  ...
  registrasi() {
    this.Next(this.params.idPasien)
  }
  Next(id) {
    // debugger
    this.cacheHelper.set('CacheRegisOnlineBaru', undefined);
    if (this.item.noReservasi != undefined) {
      var da = this.item.noReservasi
      if (this.cacheHelper.get('CacheRegisOnline') != undefined) {
        var data = this.cacheHelper.get('CacheRegisOnline')
        da = data[0];
      }
      var cache = {
        0: da,
      ...
      this.cacheHelper.set('CacheRegisOnlineBaru', cache);
    }
    this.cacheHelper.set('CacheRegisOnline', undefined);
    this.cacheHelper.set('CacheRegistrasiPasien', undefined);
    if(this.idPasien != undefined &&this.idPasien != '' ){
      this.cacheHelper.set('cacheStatusPasien', 'LAMA');
    }else{
      this.cacheHelper.set('cacheStatusPasien', 'BARU');
    }
  
    if (this.isPenunjang != undefined && this.isPenunjang == true) {
      this.cacheHelper.set('isPenunjang', true);
    } else {
      this.cacheHelper.set('isPenunjang', undefined);
    }
    this.router.navigate(['registrasi-ruangan', id])
  } 
}
