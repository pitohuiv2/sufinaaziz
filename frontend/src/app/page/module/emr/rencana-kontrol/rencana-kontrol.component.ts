...
@Component({
  selector: 'app-rencana-kontrol',
  templateUrl: './rencana-kontrol.component.html',
  styleUrls: ['./rencana-kontrol.component.scss']
})
export class RencanaKontrolComponent implements OnInit {
  itemV2: any = {
    tipe: "1"
  }
  index: any = 0
  ...
  ngOnInit(): void {
    ...
    this.item.sep = this.headData.nosep
    this.item.noKartu = this.headData.nokepesertaan
    ...
    this.item.tipe = '1'
    this.item.filter = this.listFilter[1]
    this.column = [
    ...
    this.clickRad(this.item.jenisPelayanan)
    this.cariRencana()
  }
  clickRad(e) {
    if (e == '2') {
  ...
  post(url, method, jsonView) {
    ...
    this.apiService.postNonMessage("bridging/bpjs/tools", json).subscribe(e => {
      if (e.metaData.code == "200") {
        ...
        if (this.item.noSuratKontrol) {
          this.resCetak = this.item.noSuratKontrol
        } else {
  ...
  cariRencana() {
    ...
    this.apiService.postNonMessage("bridging/bpjs/tools", json).subscribe(e => {
      ...
      if (e.metaData.code == "200") {
        for (let x = 0; x < e.response.list.length; x++) {
          const element = e.response.list[x];
          if (this.item.noKartu == element.noKartu) {
