...
@Component({
  selector: 'app-order-lab',
  templateUrl: './order-lab.component.html',
  styleUrls: ['./order-lab.component.scss']
})
export class OrderLabComponent implements OnInit, AfterViewInit {
  item: any = {
  ...
  ngOnInit(): void {
    ...
    this.apiService.get("emr/get-combo-penunjang?departemenfk=kdDepartemenLab").subscribe(dat => {
  ...
  getSelected() {
    if (this.item.layanan.length > 0) {
        ...
        if (this.item.layanan[parseInt(element)] == true) {
          for (var i = 0; i < this.produkDef.length; i++) {
            ...
            if (element2.id == element) {
              ...
              this.listChecked.push({ namaproduk: element2.namaproduk, hargasatuan: element2.hargasatuan })
            }
          }
        } else {
          for (var i = 0; i < this.produkDef.length; i++) {
      ...
      this.totalHarga = 0
      for (let x = 0; x < this.listChecked.length; x++) {
        const element = this.listChecked[x];
        this.totalHarga = this.totalHarga + parseFloat(element.hargasatuan)
      }
  ...
  save() {
    ...
    this.apiService.post('emr/save-order-pelayanan', objSave).subscribe(e => {
      ...
      this.isSimpan = false
      this.apiService.postLog('Order Laboratorium', 'Norec strukorder_t', e.strukorder.norec, 'Order Laboratorium No Order - '
  ...
  loadRiwayat(params) {
    this.apiService.get('emr/get-riwayat-order-penunjang?' + params + '&setting=kdDepartemenLab').subscribe(e => {
