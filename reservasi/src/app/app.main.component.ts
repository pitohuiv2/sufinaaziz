...
export class AppMainComponent implements AfterViewInit, OnDestroy, OnInit {
    ...
    constructor(public renderer: Renderer2, private primengConfig: PrimeNGConfig, public app: AppComponent,
		...
		// .subscribe(
		// 	(data: any) => {
        //         localStorage.setItem('xtknazdajsa',JSON.stringify(data))
        //         // console.log(data)
        //     })
    ...
    onMouseEnter(e) {
        let local = JSON.parse(localStorage.getItem('loginReservasi'))
        let session = JSON.parse(sessionStorage.getItem('loginReservasi'))
        if (!local && !session && this.router.url != '/reservasi') {
            this.display = true
        } else {
